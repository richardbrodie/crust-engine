...
impl LineSegment {
    pub fn points(&self) -> Vec<Point> {
        ...
        let n: f64 = diag_dist(self.0, self.1);
        for step in 0..n as usize {
            let t = if n == 0.0 { 0.0 } else { step as f64 / n };
            let p = lerp_point(self.0, self.1, t);
    ...
    pub fn intersects(&self, other: &Self) -> bool {
        ...
        if cmpxr == 0.0 {
            return ((other.0.x - self.0.x < 0.0) != (other.0.x - self.1.x < 0.0))
                || ((other.0.y - self.0.y < 0.0) != (other.0.y - self.1.y < 0.0));
        }
    ...
    pub fn closest_point(&self, p: Point) -> Point {
        let sv = self.1 - self.0;
        let l2 = sv.length_sq();
        if l2 == 0.0 {
            return (p - self.0).into();
        }
        let ps = p - self.0;
...
pub fn line(a: Point, b: Point) -> LineSegment {
    LineSegment(a, b)
}
...
mod tests {
    use crate::geometry::{
        line::{lerp, lerp_point, line, LineString},
        point,
    };
    ...
    fn test_closest_point() {
        ...
        let l1 = line(point(10.0, 10.0), point(40.0, 10.0));
        let p1 = point(20.0, 30.0);
        assert_eq!(l1.closest_point(p1), point(20.0, 10.0));

        let l1 = line(point(10.0, 10.0), point(40.0, 40.0));
        let p1 = point(30.0, 40.0);
        assert_eq!(l1.closest_point(p1), point(35.0, 35.0));

        let l1 = line(point(40.0, 40.0), point(10.0, 10.0));
        let p1 = point(30.0, 40.0);
        assert_eq!(l1.closest_point(p1), point(35.0, 35.0));

        let l2 = line(point(88.0, 1111.0), point(555.0, 22.0));
    ...
    fn test_lines_intersect() {
        let l1 = line(point(0.0, 0.0), point(2.0, 8.0));
        let l2 = line(point(8.0, 0.0), point(0.0, 20.0));
        assert!(!l1.intersects(&l2));

        let l1 = line(point(0.0, 10.0), point(2.0, 0.0));
        let l2 = line(point(10.0, 0.0), point(0.0, 5.0));
        assert!(l1.intersects(&l2));

        let l1 = line(point(0.0, 0.0), point(0.0, 10.0));
        let l2 = line(point(2.0, 0.0), point(2.0, 10.0));
        assert!(!l1.intersects(&l2));

        let l1 = line(point(0.0, 0.0), point(5.0, 5.0));
        let l2 = line(point(2.0, 0.0), point(7.0, 5.0));
        assert!(!l1.intersects(&l2));

        let l1 = line(point(0.0, 0.0), point(5.0, 5.0));
        let l2 = line(point(2.0, 2.0), point(7.0, 7.0));
        assert!(l1.intersects(&l2));

        let l1 = line(point(0.0, 0.0), point(5.0, 5.0));
        let l2 = line(point(7.0, 7.0), point(10.0, 10.0));
        assert!(!l1.intersects(&l2));

        let l1 = line(point(4.0, 0.0), point(6.0, 10.0));
        let l2 = line(point(0.0, 3.0), point(10.0, 7.0));
        assert!(l1.intersects(&l2));

        let l1 = line(point(0.0, 0.0), point(1.0, 1.0));
        let l2 = line(point(1.0, 2.0), point(4.0, 5.0));
        assert!(!l1.intersects(&l2));
    }
